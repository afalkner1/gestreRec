...
# vids = [{"cID": "qZEElv92rLM", "title": "Onda Onda", "cstart": "35137", "cend": "42571"}]
import pafy, cv2, json, math, multiprocessing
import sys
...
class Vid_Stream():
    ...
    def extract(self):
        for vid in self.vids:
            ...
            self.cap.set(cv2.CAP_PROP_POS_MSEC, int(vid['cstart']))
...
class Detect():
    ...
    def start(self,YT_watchID,variable):
        self.newStartupThread = threading.Thread(target = self.detect, args = [variable,YT_watchID])
...
def average(list):
    averageS = round((sum(list)/len(list))*10,1)
    return averageS

# options = {'side':'right', 'save_vid':True}
# vid_processor = Vid_Stream(vids,'hitit', options=options)
# vid_processor.train_model()
# detector = Detect()
# Detect('Fortnite_Emotes',source = 'qZEElv92rLM')
#
# with open('hitit' + '_body_language.pkl', 'rb') as f:
#     model = pickle.load(f)
